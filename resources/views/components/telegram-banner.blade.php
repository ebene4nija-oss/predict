...
@php
    ...
    $enabled = Setting::get('telegram_banner_enabled', '1');
    $channelUrl = TelegramHandles::channelUrl();
    $supportUrl = TelegramHandles::supportUrl();
    $badge = Setting::get('telegram_banner_badge', 'OFFICIAL TELEGRAM COMMUNITY');
    $icon = Setting::get('telegram_banner_icon', '✈️');
    $headline = Setting::get('telegram_banner_headline', 'Get Instant AI Predictions & Admin Support on Telegram');
    $description = Setting::get('telegram_banner_description', 'Join our public channel for instant match alerts or contact live admin support anytime.');
    $ctaText = Setting::get('telegram_banner_cta_text', 'Join VIP Channel');
    $supportCtaText = Setting::get('telegram_support_cta_text', '💬 Admin Support');
@endphp

@if($enabled != '0' && ($channelUrl !== '' || $supportUrl !== ''))
    ...
    <div class="flex flex-col md:flex-row items-center justify-between gap-4 relative z-10">
        <div class="flex items-center space-x-4">
            ...
            <div>
                ...
                @endif
                <h3 class="text-base sm:text-lg font-black text-white">{{ $headline }}</h3>
