...
/**
 * Google Gemini-backed match analysis.
 *
 * Calls Gemini API with JSON output mode to estimate market probabilities
...
class GeminiPredictionService
{
    ...
    {
        return Setting::credential('gemini_prediction_model', 'services.gemini.prediction_model')
            ?: (Setting::credential('gemini_model', 'services.gemini.model') ?: self::DEFAULT_MODEL);
    }
    ...
    public function generate(GameMatch $match): ?array
    {
        ...
        try {
            $response = Http::timeout(30)->post(
                ...
                [
                    'systemInstruction' => [
                        'parts' => [
                            [
                                'text' => 'You are a quantitative football analyst. Estimate genuine probabilities '
                                    ...
                                    .'Return a single JSON object where each key is the market name and the value is an object with "pick", "probability", and "rationale".',
                            ],
                        ],
                    ],
                    'contents' => [
                        [
                            'parts' => [
                                ['text' => $this->prompt($match, $markets)],
                            ],
                        ],
                    ],
                    'generationConfig' => [
            ...
            if (! $response->successful()) {
                Log::error('Gemini prediction request failed', [
                    'match_id' => $match->id,
            ...
            if (empty($text)) {
                Log::warning('Gemini returned empty prediction response', ['match_id' => $match->id]);
            ...
            if (! is_array($payload)) {
                Log::warning('Gemini returned non-JSON payload', ['match_id' => $match->id, 'text' => $text]);
            ...
            Log::error('Gemini prediction transport failure', [
                'match_id' => $match->id,
    ...
    protected function normalise(array $payload, GameMatch $match, array $expected): ?array
    {
        ...
        foreach ($expected as $market) {
            ...
            if ($probability <= 0.0 || $probability > 1.0) {
                Log::warning('Gemini returned an out-of-range probability', [
                    'match_id' => $match->id,
                    'market' => $market,
            ...
            $markets[$market] = [
                'pick' => $pick,
                'probability' => $probability,
                'rationale' => trim((string) ($entry['rationale'] ?? '')) ?: 'Gemini analysis.',
            ];
        }

        if ($markets === []) {
            Log::warning('Gemini response contained no usable markets', ['match_id' => $match->id]);
